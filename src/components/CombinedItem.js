  ...
  return (
		<>
			<CombineWrap className="wrap" backImg={imgSet.back[2]} >
				<div className="combineItem_area">
					<div className="combineItem_box num4">
						{selectItem.save && selectItem.save.map((data, idx) => {
							...
							return (
								<div className={`item_layout ${cate ? gameData.itemGrade.txt_e[data.grade || items.grade].toLowerCase() : ''} ${selectIdx === idx ? 'select' : ''}`} key={`items${idx}`} onClick={() => {
									setSelectIdx(idx);
				...
				<div className="combineItem_area">
					<div className="combineItem_bottom num6 scroll-y">
