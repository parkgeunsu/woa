export const skill = [
	...
	{idx:259,
		...
		multiplesAttack:[],
		eff:[{type:5,num:['70%','80%','90%','100%','110%']}],buff:[{type:51,num:[]}],buffCount:[],buffChance:['2%','4%','6%','8%','10%'],atkCount:[1],sp:15},
	{idx:260,
	...
	{idx:270,
		na:{ko:'추격하기',en:'Catch up',jp:'追いかける'},element_type:0,cate:[8],
	...
	{idx:272,
		...
		txt:'<area>, <dmg>, <el>',
		ta_:0,ta:[27,27,37,37,20],effAnimation:'custom01',effAnimationRepeat:1,effSize:[5,5,5,5,5],effRotate:0,effFilter:'',
		multiplesAttack:[{type:0,num:['125%','130%','135%','140%','150%']}],
		eff:[{type:3,num:['120%','130%','140%','150%','170%']}],atkCount:[1],sp:24},
	'','','','','','','','',//280
	'','','','','','','','','','',//290
	'','','','','','','','','','',//300
	{idx:237,
...
// [eff], [buff] 체력HP(0), 행동SP(1), 행동회복RSP(2), 공ATK(3), 방DEF(4), 술공MAK(5), 술방MDF(6), 회복RCV(7), 속도SPD(8), 행운LUK(9), 카운터Atk(10), 쪼기(11), 할퀴기(12), 물기(13), 치기(14), 누르기(15), 던지기(16), 빛(17), 어둠(18), 물(19), 불(20), 바람(21), 땅(22), 출혈(50), 중독(51), 석화(52), 혼란(53), 기절(54), 변이(55), 즉사(56), 패시브(100)
// [ta_] 아군0, 적군1
// [ta] getEffectArea 효과범위, passive일 경우 1:단일, 100:직업
// 영역 1단일, 2가로2, 3가로3, 4세로2, 5세로3, 6가로행, 7세로열, 8십자5, 9십자9, 10대각선/, 11대각선\, 12고정세로2열, 13고정세로3열, 15└┐, 16┌┘, 17卍, 18가로2행, 19가로3행, 20전체, 21정사각형9, 22정사각형4, 23자신, 24원, 25랜덤5, 26랜덤10, 27랜덤15, 28작은마름모, 29큰마름모, 30큰링, 31랜덤세로2열, 32랜덤세로3열, 33랜덤가로2행, 34랜덤가로3행, 35x자5, 36x자9, 37랜덤20,100고양이, 101사자, 102호랑이, 103개, 104늑대, 105물개, 106너구리, 107쥐, 108토끼, 109원숭이, 110고릴라, 111캥거루, 112소, 113곰, 114말, 115사슴, 116코뿔소, 117코끼리, 118기린, 119새, 120독수리, 121뱀, 122도마뱀, 123거북이, 124개구리, 125돼지
...
// 물기, 치기
// 영역표시(buff)●, 냄새맡기(buff)●, 
// 뒷발차기2(치기)●, 송곳니뚫기(물기)●, 물어뜯기(물기)●, 무리사냥2(바람,광)●, 광견병(독, 광역)●, 추격하기(반격기), 

// * 4늑대 - 늑대의 강인한 의리(passive), 야행성(속도 condition)●, 개과(passive)
// 물기, 치기
// 영역표시(buff)●, 하울링(buff), 냄새맡기(buff)●, 집념의추적(buff),
// 뒷발차기2(치기)●, 송곳니뚫기(물기)●, 물어뜯기(물기)●, 무리사냥1(땅,광)●, 무리사냥2(바람,광)●, 광견병(독, 광역)●, 뼈아작내기(물기),
// [유혹(여우한정)], 

// * 5물개 - 물개의 유연함(passive),
// 치기, 누르기
// 회전박치기(바람)(세로 일자영역), 물대포(수),
// [송곳니뚫기(바다사자특정)],

// * 6너구리 - 너구리의 영특한 계산(passive), 반격(passive),
// 물기, 치기, 던지기
// 돌던지기(땅), 고드름던지기(수), 찹찹(회복,자신),
...
// 쪼기, 치기
// 손톱&뿔 갈기(buff)●,
// 뿔찌르기(쪼기), 정화(상태이상치료), 그룹정화(상태이상치료), 뒷발차기1(치기)●, 돌진1(광역, 치기)
...
// 누르기, 치기
// 기침(buff),
// 목치기(치기), 뒷발차기2(치기)●, 돌진2(광역, 치기), 머리박치기(치기), 초저주파(광역, 빛), 발구르기(광역, 땅)
