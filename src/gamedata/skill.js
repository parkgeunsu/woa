...
export const skill = [
	...
	{idx:115,
		...
		,eff:[{type:3,num:['10%','130%','150%','170%','200%']}],buff:[{type:50,num:['-100','-200','-300','-400','-500']}],buffCount:[3,3,3,3,3],buffChance:['60%','70%','80%','90%','90%'],atkCount:[1],turn:2,sp:5},
//---------------------------------------------------
	{idx:116,
	...
	{idx:121,
		...
		,ta_:1,ta:1,effAnimation:5,buffAnimation:5,skillClass:1
		,eff:[{type:3,num:['10%','100%','150%','170%','200%']}],buff:[{type:51,num:['-50','-100','-300','-400','-500']}],buffCount:[2,2,2,2,2],buffChance:['70%','75%','80%','85%','90%'],atkCount:[1],turn:2,sp:5},
	{idx:122,
...
// 동물타입
// * 0고양이 - 고양이의 예민한 직감(passive),하악질, 꾹꾹이, 그루밍, 양손타격1, 한손타격1, 뒷발치기1, 빵굽기2, 할퀴기, 물기

// * 1사자 - 사자의 빛나는 영광(passive), 한손타격2, 양손타격1, 포효, 송곳니뚫기, 금빛갈기(buff), 할퀴기, 밀림의왕(passive), 무리 사냥(광), 하악질, 빵굽기1, 뒷발치기1, 물기

// * 2호랑이 - 호랑이의 거친 용맹(passive), 양손타격2, 한손타격1, 포효, 송곳니뚫기, 앞발 휘두르기(광역), 뒷발치기1, 잠행(buff), 정글의왕(buff), 피의 서약, 하악질, 빵굽기1, 물기

// * 3강아지 - 강아지의 충성스러운 용기(passive), 송곳니뚫기, 물어뜯기, 발달된후각, 광견병(광역)

// * 4늑대 - 늑대의 강인한 의리(passive), 하울링, 송곳니뚫기, 물어뜯기, 집념의추적(buff), 달빛소나타(passive), 발달된후각, 뼈씹기, [유혹(여우한정)], 광견병(광역)

// * 5물개 - 물개의 유연한 가르기(passive), [송곳니뚫기(바다사자특정)], 

// * 6너구리 - 너구리의 영특한 계산(passive), 고드름던지기(수), [주머니털기(너구리한정)], [조개 선물(회복)(해달한정)], 찹찹(회복,자신), 물기

// * 7쥐 - 쥐의 민첩한 은든술(passive), 물기

// * 8토끼 -  토끼의 뛰어난 점프력(spd, luk)(passive), 뒷발치기1, 물기

// * 9원숭이 - 원숭이의 영리한 지략(passive), 침뱉기(수, 광), 돌던지기(땅), 고드름던지기(수), [온천반신욕(일본원숭이한정)], 조롱하기

// * 10고릴라 - 고릴리의 막강한 파괴(atk)(passive), 가슴치기(buff), 허리접기, 침뱉기(수, 광), 1톤 펀치, 돌던지기(땅), 바위 던지기(땅), 고드름던지기(수)

// * 11캥거루 - 캥거루의 치유의 주머니(힐 기술)(passive), 하이킥, 꼬리치기2

// * 12소 - 소의 우직한 전진(passive), 뿔찌르기, 뿔갉기, 돌진3(광역),

// * 13곰 - 곰의 우렁찬 포효(hp)(passive), 포효, 한손타격2, 양손타격2, 할퀴기, 

// * 14말 - 말의 날쌘 이동력(spd)(passive), 뒷발치기3, 질주(buff), 돌진2(광역) 

// * 15사슴 - 사슴의 성스러운 보호(mdk)(passive), 뿔찌르기, 뿔갉기, 정화(상태이상치료), 그룹정화(상태이상치료), 뒷발치기1, 돌진1(광역)
	
// * 16코뿔소 - 코뿔소의 돌진하는 용맹(passive), 코뿔박치기, 뚫기, 돌진3(광역)

// * 17코끼리 - 코끼리의 무거운 한방(광역공격위주)(passive), 돌진3(광역), 밟기, 돌던지기(땅), 바위던지기(땅), 물대포(수), 깔아뭉개기(광), 포효, 땅울림(땅, 광), 꼬리치기

// * 18기린 - 기린의 충실한 정찰력(passive), 목치기, 뒷발치기2, 돌진2(광역) 

// * 19새 - 새의 청량한 노래(passive), 쪼기1, 낙석(땅), 날개치기1(바람), 고공낙하(바람), 치유의 노래(회복, 광), 쪼기
	
// * 20독수리 - 독수리의 영리한 전략(passive), 쪼기2, 낙석(땅), 고공낙하(바람), 날개치기2(바람), 윈드커터(바람), 쪼기

// * 21뱀 - 뱀의 교활한 계략(passive), 독뿌리기(암), 감아쪼이기, 허물벗기(회복),
	
// * 22도마뱀 - 도마뱀의 뛰어난 은둔술(passive), 혓바닥공격, 허물벗기(회복), 꼬리자르기(회피), 물기

// * 23거북이 - 거북이의 강철 피부(passive), 절단물기, 튼튼한등껍질(방어), 물기,

// * 24개구리 - 개구리의 신속한 점프력(passive), 개구리 뿔피리(passive), 뒷발치기1, 개구리 기우제, [청개구리 공격(한정)], [독뿌리기(암)(한정)], 음파공격(술법,광역), 혓바닥공격, 고드름던지기(수), 점프(회피), 물기
