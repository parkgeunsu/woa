...
const WorkHeader = styled(FlexBox)`
	height: auto;
`;
const TavernContent = styled(FlexBox)`
	flex: 1;
	width: 100%;
	height: 100%;
	overflow: hidden;
...
const Tavern = ({
  ...
  const actionChIdx = React.useMemo(() => {
    return sData.actionCh.tavern.idx <= entries.length - 1 ? sData.actionCh.tavern.idx : "";
  }, [entries, sData]);
  const saveCh = React.useMemo(() => entries[actionChIdx] || {}, [entries, actionChIdx]);
  const [greeting, setGreeting] = useState(gameData.shop.tavern.greeting[lang]);
  useEffect(() => {
    setLoading(false);
  }, []);
  return (
    <>
			<Wrap direction="column">
				<Npc imgSet={imgSet} shopType={'tavern'} gameData={gameData} lang={lang} selectTab={selectTab} setSelectTab={setSelectTab} navigate={navigate} onClick={() => {
          setSelectTab("");
          const randomIdx = Math.floor(Math.random() * gameData.shop.tavern.randomText.length);
          setGreeting(gameData.shop.tavern.randomText[randomIdx][lang]);
				}}/>
        <WorkArea frameBack={imgSet.etc.frameChBack}direction="column" alignItems="center" justifyContent="center">
          ...
          {selectTab === 0 && <TavernContent direction="row" justifyContent="center" alignItems="flex-start" onClick={() => {
            }}>
        ...
        <UserContainer justifyContent="space-between">
          ...
          <ActionPic onClick={() => {
            ...
            <ActionChDisplay type={'tavern'} saveData={sData} gameData={gameData} actionChIdx={actionChIdx} imgSet={imgSet}/>
          </ActionPic>
        </UserContainer>
      </Wrap>
			<PopupContainer>
