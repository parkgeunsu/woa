...
const ItemEnhance = styled.div`
  ...
  border-image: url(${frameChBack}) 5 round;
  z-index: 3;
  & {
    ...
    .hole_item .item_lv { position: absolute; left: 0; top: 0; padding: 5px; z-index: 2; background: rgba(0,0,0,0.5); text-align: left; font-size: 0.875rem; font-weight: 600; }
    ...
    .has_items { flex: 0.7; margin: 0 0 0 5px; }
    ...
    .has_stones { flex: 0.7; margin: 0 0 0 5px; }
            ...
            <div className="has_items scroll-y">
              <ul className="h_items">
                {Array.from({ length: 28 }).map((_, i) => (
                  <li key={`item-${i}`} className={`item${(i % 5) + 1}`}></li>
                ))}
              </ul>
            </div>
            <div className="has_stones scroll-y">
              <ul className="h_items">
                {Array.from({ length: 41 }).map((_, i) => (
                  <li key={`stone-${i}`} className={`item${(i % 5) + 1}`}></li>
                ))}
              </ul>
            </div>
          </dd>
