...
const WorkHeader = styled(FlexBox)`
	height: auto;
`;
const PortContent = styled(FlexBox)`
	flex: 1;
	width: 100%;
	height: 100%;
	overflow: hidden;
...
const Port = ({
  ...
  const actionChIdx = React.useMemo(() => {
    return sData.actionCh.port.idx <= entries.length - 1 ? sData.actionCh.port.idx : "";
  }, [entries, sData]);
  const saveCh = React.useMemo(() => entries[actionChIdx] || {}, [entries, actionChIdx]);
  const [greeting, setGreeting] = useState(gameData.shop.port.greeting[lang]);
  useEffect(() => {
    setLoading(false);
  }, []);
  return (
    <>
          ...
          setSelectTab("");
          const randomIdx = Math.floor(Math.random() * gameData.shop.port.randomText.length);
          setGreeting(gameData.shop.port.randomText[randomIdx][lang]);
				}}/>
        <WorkArea frameBack={imgSet.etc.frameChBack}direction="column" alignItems="center" justifyContent="center">
          ...
          {selectTab === 0 && <PortContent direction="row" justifyContent="center" alignItems="flex-start" onClick={() => {
            }}>
        ...
        <UserContainer justifyContent="space-between">
          ...
          <ActionPic onClick={() => {
            ...
            <ActionChDisplay type={'port'} saveData={sData} gameData={gameData} actionChIdx={actionChIdx} imgSet={imgSet}/>
          </ActionPic>
        </UserContainer>
      </Wrap>
			<PopupContainer>
