...
const WorkHeader = styled(FlexBox)`
	height: auto;
`;
const GuildContent = styled(FlexBox)`
	flex: 1;
	width: 100%;
	height: 100%;
	overflow: hidden;
...
const Guild = ({
  ...
  const actionChIdx = React.useMemo(() => {
    return sData.actionCh.guild.idx <= entries.length - 1 ? sData.actionCh.guild.idx : "";
  }, [entries, sData]);
  const saveCh = React.useMemo(() => entries[actionChIdx] || {}, [entries, actionChIdx]);
  const [greeting, setGreeting] = useState(gameData.shop.guild.greeting[lang]);
  useEffect(() => {
    setLoading(false);
  }, []);
  return (
    <>
			<Wrap direction="column">
				<Npc imgSet={imgSet} shopType={'guild'} gameData={gameData} lang={lang} selectTab={selectTab} setSelectTab={setSelectTab} navigate={navigate} onClick={() => {
          setSelectTab("");
          const randomIdx = Math.floor(Math.random() * gameData.shop.guild.randomText.length);
          setGreeting(gameData.shop.guild.randomText[randomIdx][lang]);
				}}/>
        <WorkArea frameBack={imgSet.etc.frameChBack}direction="column" alignItems="center" justifyContent="center">
          ...
          {selectTab === 0 && <GuildContent direction="row" justifyContent="center" alignItems="flex-start" onClick={() => {
            }}>
        ...
        <UserContainer justifyContent="space-between">
          ...
          <ActionPic onClick={() => {
            ...
            <ActionChDisplay type={'guild'} saveData={sData} gameData={gameData} actionChIdx={actionChIdx} imgSet={imgSet}/>
          </ActionPic>
        </UserContainer>
      </Wrap>
			<PopupContainer>
