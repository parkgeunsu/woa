...
const WorkHeader = styled(FlexBox)`
	height: auto;
`;
const TownHallContent = styled(FlexBox)`
	flex: 1;
	width: 100%;
	height: 100%;
	overflow: hidden;
...
const TownHall = ({
  ...
  const actionChIdx = React.useMemo(() => {
    return sData.actionCh.townHall.idx <= entries.length - 1 ? sData.actionCh.townHall.idx : "";
  }, [entries, sData]);
  const saveCh = React.useMemo(() => entries[actionChIdx] || {}, [entries, actionChIdx]);
  const [greeting, setGreeting] = useState(gameData.shop.townHall.greeting[lang]);
  useEffect(() => {
    setLoading(false);
  }, []);
  return (
    <>
          ...
          setSelectTab("");
          const randomIdx = Math.floor(Math.random() * gameData.shop.townHall.randomText.length);
          setGreeting(gameData.shop.townHall.randomText[randomIdx][lang]);
				}}/>
        <WorkArea frameBack={imgSet.etc.frameChBack}direction="column" alignItems="center" justifyContent="center">
          ...
          {selectTab === 0 && <TownHallContent direction="row" justifyContent="center" alignItems="flex-start" onClick={() => {
            }}>
        ...
        <UserContainer justifyContent="space-between">
          ...
          <ActionPic onClick={() => {
            ...
            <ActionChDisplay type={'townHall'} saveData={sData} gameData={gameData} actionChIdx={actionChIdx} imgSet={imgSet}/>
          </ActionPic>
        </UserContainer>
      </Wrap>
			<PopupContainer>
